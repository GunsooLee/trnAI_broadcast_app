...
- **📊 데이터 기반 분석**: 과거 방송 실적, 날씨, 기념일, 외부 최신 트렌드 키워드를 종합하여 AI가 구체적이고 설득력 있는 추천 근거를 자동 생성합니다.
...
- **🤖 AI 추천 근거 자동 생성**: LangChain을 활용하여 각 추천 상품에 대한 매출 전망, 경쟁 분석, 트렌드 연관성을 포함한 상세하고 전문적인 근거를 자동 생성합니다.
...
4. **AI 생성 추천 근거 확인**: 각 상품별로 AI가 자동 생성한 상세하고 전문적인 추천 근거(매출 전망, 경쟁 분석, 트렌드 연관성, 시간대 적합성)를 확인합니다.
...
선정된 상품에 대해 **왜 이 상품을 추천했는지** 구체적이고 설득력 있는 근거를 자동 생성합니다.
...
- **출력**: "'주방용품' 카테고리의 예상 매출이 높고, 동시간대 경쟁이 없어 독점 방송이 가능합니다."

**폴백 처리:** API 오류 시 기본 템플릿(`"'{카테고리}' 카테고리의 베스트셀러 상품입니다."`) 사용하여 시스템 안정성 보장
...
#### AI 기반 추천 근거 생성
- 선정된 각 상품에 대해 **LangChain을 활용한 상세 추천 근거**를 자동 생성합니다.
- 매출 전망, 경쟁 상황, 트렌드 연관성, 시간대 적합성을 종합하여 구체적이고 설득력 있는 설명을 제공합니다.
- API 오류 시 기본 템플릿으로 폴백하여 시스템 안정성을 보장합니다.

#### API 응답 생성
- 최종 점수가 높은 순서대로 상품을 정렬하고, AI 생성 추천 근거와 함께 JSON 형식으로 응답을 생성합니다.
